...
function checkLine(values: number[][], x: number, y: number, dx: number, dy: number) {
    let l = 4 - 1 
    let N = values.length

    // if we are going to hit an edge of the matrix return false
    let xedge = x + (dx * l)
    let yedge = y + (dy * l)
    if (xedge < 0 || xedge >= N || yedge < 0 || yedge >= N) {
        return false
    }
...
function findSequence(values: number[][]): boolean {
    console.log(values.map((v: number[]): string => v.join("")).join("-"))
    let N = values.length
    for (let x = 0; x < N; x++) {
        for (let y = 0; y < N; y++) {
            if (checkLine(values, x, y, 1, 0)) {
                return true
            }
            if (checkLine(values, x, y, 0, 1)) {
                return true
            }
            if (checkLine(values, x, y, 1, 1)) {
                return true
            }
            if (checkLine(values, x, y, 1, -1)) {
                return true
